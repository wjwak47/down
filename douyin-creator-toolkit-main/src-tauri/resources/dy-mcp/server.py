...
def generate_jianying(
    ...
    if not os.path.isdir(TUTORIAL_DIR):
        raise Exception(f"素材目录不存在: {TUTORIAL_DIR}")

    if not os.path.isdir(draft_folder):
        raise Exception(f"剪映草稿目录不存在: {draft_folder}")
    ...
    # ----- 7. GIF -----
    gif_path = os.path.join(TUTORIAL_DIR, "sticker.gif")
    gif_material = draft.VideoMaterial(gif_path)
    ...
    return f"剪映草稿已生成: {draft_name} 目录: {draft_folder}"
